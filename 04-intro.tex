...
(\autoref{sec:relatedWorkTisane}), this chapter describes the \tisane DSL
(\autoref{sec:tisane}) and case studies evaluating \tisane
(\autoref{sec:tisane_case_studies}). 

\autoref{chapter:rTisane} starts with a study of how statistical non-experts
want to express their implicit domain knowledge
(\autoref{sec:exploratoryStudy}). Then, the chapter proceeds with design goals
and system implementation details for \rTisane (\autoref{sec:rTisane}). This
chapter concludes with a controlled lab study evaluating \rTisane
