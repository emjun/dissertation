...
Our aim was to understand what challenges statistical non-experts face when
externalizing their implicit domain knowledge. These challenges could then, in
turn, inform improvements to \tisane's DSL. Thus, we used the first release of
\tisane~\cite{jun2022tisane} to probe into analysts' internal processes and derive
design goals~\autoref{sec:rtisane_design_implications} for designing \rTisane\footnote{For the lab study, we re-implemented \tisane (originally in Python) in R since
(i) R is a widely used programming language for data science and (ii) the
research methods course from which we recruited participants used R.}.
% Our aim with \rTisane, the R version
% of \tisane, was to increase the expressivity of the DSL to represent analysts'
% implicit domain knowledge. We used 
...
\section{Design Goals} \label{sec:rtisane_design_implications} 
