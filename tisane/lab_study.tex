...
scale. \polish{Participants also self-rated their data analysis experience at a median of eight out of ten (min: 5, max: 10).}~\autoref{tab:summativeEvaluationParticipants} summarizes the
...
\subsection{Findings}
\polish{Add table/figure with quantitative model + AIC/BIC comparisons}
\tableModelScores
...
% personal opinion) was still the same (with or without rTisane).} 
\polish{Missing connective text before "expand upon on [sic] their own"}
For P5 and P7,
...
author their own. \polish{Not super clear. What were the relevant instructions? Did P11 base their "own" script on prior rTisane output? If so, make this clearer.} After observing the model's \shortquote{AIC is large, the residual is
...
\polish{Add table to compare AIC/BIC}
...
and link functions [P2, P4, P5, P9, P10, P11]. P4 explained, \shortquote{I
...
saying, \shortquote{Looking at the \texttt{summary()} in R was too much to look at.} He suggested a
simple way to tie the results back to his input conceptual model: \shortquote{Would be nice
...
described the \rTisane output statistical model as \shortquote{an initial or baseline model
but follow-up evaluation of the model is needed.} They wanted to \shortquote{go back and
...
outside their IDE complicated and \shortquote{clunky.} While part of this may have been
