...
\subsection{\rTisane's DSL}
Like \tisane, analysts express variables, a conceptual model, and a query for a
statistical model. \rTisane's DSL prioritizes expressivity and usability 
...
through a Unit. \polish{For example, reference prog} Measures can be one of four
types: continuous, unordered categories (i.e., nominal), ordered categories
(i.e., ordinal), and counts. \rTisane provides syntactic sugar for declaring
...
variable relationships to it (\polish{ref prog}). The conceptual model %\texttt{ConceptualModel}
...
unidirectional influence from a cause to an effect. \polish{Ref prog}. \causes
...
functions. \polish{Ref prog}. There are four comparisons analysts can specify in
...
\hypothesize a relationship. \polish{ref program}.
...
model (\polish{ref prog}). The query asks for a statistical model to accurately
...
To disambiguate conceptual models, \rTisane uses a GUI. The GUI shows a graph
representing analysts' conceptual models (\polish{ref fig}). If there are any \relates
relationships, \rTisane suggests ways analysts could assume a direction of
influence (\polish{ref fig}). Additionally, \rTisane suggests ways to break any cycles in the
conceptual model (\polish{ref fig}). As analysts make changes, the visible graph updates. The GUI
